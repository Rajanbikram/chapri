...
const Register = () => {
  ...
  const iw = { display: "flex", alignItems: "center", gap: 9, border: "1.5px solid #ddd", borderRadius: 9, padding: "11px 14px" };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,40,110,.75) 0%, transparent 60%)" }} />
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <h2 style={{ fontSize: isMobile ? "1.4rem" : "1.85rem", fontWeight: 800, lineHeight: 1.2, marginBottom: isMobile ? 4 : 10 }}>
              Start your journey<br/>with us today.
            </h2>
            {!isMobile && (
              <>
                <p style={{ fontSize: ".87rem", color: "rgba(255,255,255,.8)", lineHeight: 1.65, marginBottom: 28 }}>
                  Join a global community of over 2 million travelers and aviation professionals.
                </p>
                <div style={{ display: "flex", gap: 28 }}>
                  {[["150+", "Countries"], ["2.4k", "Airlines"], ["12M", "Bookings"]].map(([n, l]) => (
                    <div key={l}>
                      <div style={{ fontSize: "1.3rem", fontWeight: 800 }}>{n}</div>
                      <div style={{ fontSize: ".72rem", color: "rgba(255,255,255,.65)", marginTop: 2 }}>{l}</div>
                    </div>
                  ))}
                </div>
              </>
          ...
          {!isMobile && (
            <div style={{ position: "absolute", bottom: 12, left: 0, right: 0, textAlign: "center", fontSize: ".7rem", color: "rgba(255,255,255,.45)", zIndex: 1 }}>
              © 2024 SkyBound Aviation Systems.
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          padding: isMobile ? "28px 24px 40px" : "36px 44px",
          display: "flex", flexDirection: "column", justifyContent: "center",
          overflowY: "auto"
        }}>
          <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#111", marginBottom: 4 }}>Create your account</h1>
          ...
          <div style={{ display: "flex", border: "1.5px solid #ddd", borderRadius: 9, overflow: "hidden", marginBottom: 8 }}>
            {[["traveler", "Traveler"], ["airline_owner", "Airline Owner"]].map(([val, label]) => (
              <button key={val} onClick={() => setForm({ ...form, role: val })} style={{
                ...
                background: form.role === val ? "#1d6af4" : "#fff",
                color: form.role === val ? "#fff" : "#777",
                cursor: "pointer", fontSize: ".87rem",
          ...
          {error && (
            <div style={{ background: "#fff5f5", border: "1px solid #fc8181", color: "#c53030", padding: "10px 14px", borderRadius: 8, marginBottom: 10, fontSize: ".87rem" }}>
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={iw}>
              <svg width="16" height="16" fill="none" stroke="#aaa" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="7" r="4"/><path d="M4 21c0-4 3.6-7 8-7s8 3 8 7"/></svg>
              <input type="text" name="name" placeholder="John Doe" value={form.name} onChange={handleChange} required style={inp}/>
            </div>
            ...
            <div style={iw}>
              ...
              <input type="email" name="email" placeholder="john@example.com" value={form.email} onChange={handleChange} required style={inp}/>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14 }}>
              <div>
                ...
                <div style={iw}>
                  ...
                  <input type={showPw ? "text" : "password"} name="password" placeholder="••••••••" value={form.password} onChange={handleChange} required style={inp}/>
              ...
              <div>
                ...
                <div style={iw}>
                  <svg width="16" height="16" fill="none" stroke="#aaa" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                  <input type={showPw2 ? "text" : "password"} name="confirmPassword" placeholder="••••••••" value={form.confirmPassword} onChange={handleChange} required style={inp}/>
                  <span onClick={() => setShowPw2(!showPw2)} style={{ cursor: "pointer", color: "#aaa" }}>
                    <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                  </span>
                </div>
              </div>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginTop: 14, fontSize: ".83rem", color: "#666" }}>
              <input type="checkbox" checked={agree} onChange={e => setAgree(e.target.checked)}
                style={{ accentColor: "#1d6af4", width: 15, height: 15, cursor: "pointer", marginTop: 2, flexShrink: 0 }} />
              <span>I agree to the <a href="#" style={{ color: "#1d6af4", fontWeight: 600, textDecoration: "none" }}>Terms of Service</a> and <a href="#" style={{ color: "#1d6af4", fontWeight: 600, textDecoration: "none" }}>Privacy Policy</a></span>
            </div>
